...
public class AbstractMockCrudRepository<T, ID extends Long> implements CrudRepository<T, ID> {
    ...
    protected AbstractMockCrudRepository(Class<T> clazz) {
        this.clazz = clazz;
        ...
        relations = Arrays.stream(declaredFields).filter(AbstractMockCrudRepository::isRelation).peek(f -> f.setAccessible(true)).collect(Collectors.toList());
    }
            ...
            relations.forEach(r -> {
                try {
                    if (r.isAnnotationPresent(OneToMany.class)) {
                        ...
                        AbstractMockCrudRepository mockCrudRepository = new AbstractMockCrudRepository(type);
